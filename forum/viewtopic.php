...
if (!$topic_id && !$post_id)
{
	handle_topic_not_found($post_id, $topic_id, $forum_id);
}
...
$posts_per_page = max(5, $posts_per_page);
$posts_per_page = min(200, $posts_per_page);

// What is start equal to?
if ($post_id)
{
	$start = floor(($topic_data['prev_posts']) / $posts_per_page) * $posts_per_page;
}
...
{
	$start = ($start < 0) ? 0 : floor(($total_posts - 1) / $posts_per_page) * $posts_per_page;
}
...
if (($config['email_enable'] || $config['jab_enable']) && $config['allow_topic_notify'] && $user->data['is_registered'])
{
 ...
 // Activity overview
 if( request_var('activity_overview', 0) )
 {
	$sort_type = request_var('sort_type', "");
...
$template->assign_vars(array(
	...
	'WIKI_IMG' 			=> $user->img('icon_contact_wiki', 'WIKI'),
	...
	'U_REPLY_ONCLICK' => $auth->acl_get('f_reply', $forum_id) ? !$user->optionget('allow_quote_options') && $config['enable_multiquote'] ? "return replyquote($topic_id, $forum_id)": '' :'',
        'U_ACTIVITY_OVERVIEW'            => (append_sid("{$phpbb_root_path}viewtopic.$phpEx", "f=$forum_id&amp;t=$topic_id&amp;activity_overview=1")), 
...
if ($start > $total_posts / 2)
{
	...
	if ($start + $posts_per_page > $total_posts)
	{
		$sql_limit = min($posts_per_page, max(1, $total_posts - $start));
	}
	...
	if (!isset($user_cache[$poster_id]))
	{
		if ($poster_id == ANONYMOUS)
		{
			$user_cache[$poster_id] = array(
				...
				'warnings'			=> 0,
				'allow_pm'			=> 0,
				// viewtopic birthday/scumday
				'viewtopic_birthday'		=> false,
				'viewtopic_scumday'		=> false,
			);
		...
		else
		{
			...
			$user_cache[$poster_id] = array(
				'joined'		=> $user->format_date($row['user_regdate'], 'F d, Y'),
				...
				'facebook'		=> ($row['user_facebook']) ? 'https://www.facebook.com/' . urlencode($row['user_facebook']) : '',
				'twitter'		=> ($row['user_twitter']) ? 'https://twitter.com/' . urlencode($row['user_twitter']) : '',
			...
			// viewtopic scumday
			$user_cache[$poster_id]['viewtopic_scumday'] = false;
			if ($config['allow_birthdays'] && !empty($row['user_regdate']))
			{
				$userRegDate = strftime("%d-%m-%Y", $row['user_regdate']);
				list($bday_day, $bday_month) = array_map('intval', explode('-', $userRegDate));

				if ($bday_day === (int) $now['mday'] && $bday_month === (int) $now['mon'])
				{
					$user_cache[$poster_id]['viewtopic_scumday'] = true;
...
//initialize the global JS variable that tracks what posts have been multiquoted to the value pass by get at the start of the page.


	$template->assign_var('LOAD_OPTIONS', "onload=initializeMultiposts($topic_id) onbeforeunload=saveMutiquote($topic_id)");
...
for ($i = 0, $end = sizeof($post_list); $i < $end; ++$i)
{
	...
	$postrow = array(
		...
		// viewtopic scumday
		'SCUMDAY_IMG'		=> $user_cache[$poster_id]['viewtopic_scumday'] ? $user->img('icon_scumday', 'VIEWTOPIC_SCUMDAY') : false,
		...
		'U_QUOTE_NOJS'      => ($auth->acl_get('f_reply', $forum_id)) ? './posting.php?mode=quote&f=' . $forum_id . '&p=' . $row['post_id'] : '',
		...
		'U_WIKI'		=> 'http://wiki.mafiascum.net/index.php?title=' . $user_cache[$poster_id]['author_username'],
...
{
	$last_page = ((floor($start / $posts_per_page) + 1) == max(ceil($total_posts / $posts_per_page), 1)) ? true : false;
...
if ($s_can_vote || $s_quick_reply)
{
	...
	if ($s_quick_reply)
	{
		...
		$template->assign_vars(array(
			'S_QUICK_REPLY'			=> true,
			'U_QR_ACTION'			=> append_sid("{$phpbb_root_path}posting.$phpEx", 
			"mode=reply&amp;f=$forum_id&amp;t=$topic_id"),
			'QR_SUBMIT_ACTION'		=> !$user->optionget('allow_quote_options') && $config['enable_multiquote'] ? "formquote($topic_id, $forum_id, 's')" : "",
			'QR_PREVIEW_ACTION'		=> !$user->optionget('allow_quote_options') && $config['enable_multiquote'] ? "formquote($topic_id, $forum_id, 'p')": "document.getElementById('postform').action += '#preview';",
