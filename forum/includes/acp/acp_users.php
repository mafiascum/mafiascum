	...
	function main($id, $mode)
	{
		...
		switch ($mode)
		{
			case 'overview':
					...
					if (!sizeof($error))
					{
						...
						if ($update_email !== false)
						{
							$sql_ary += array(
								...
								'user_old_emails'	=> (($user_row['user_old_emails'] != '' ? ($user_row['user_old_emails'] . "\n") : "") . $user_row['user_email']),
							);
				...
				if ($user_row['user_type'] == USER_INACTIVE)
				{
					...
					switch ($user_row['user_inactive_reason'])
					{
						...
						break;
						case INACTIVE_IPFLAG:
							$inactive_reason = $user->lang['INACTIVE_REASON_IPFLAG'];
						break;
						case INACTIVE_EMAILFLAG:
							$inactive_reason = $user->lang['INACTIVE_REASON_EMAILFLAG'];
						break;
						case INACTIVE_SPAMFLAG:
							$inactive_reason = $user->lang['INACTIVE_REASON_SPAMFLAG'];
						break;
					}
				}
				...
				$template->assign_vars(array(
					...
					'SFS_APPEARS'        => (sfs_check($user_row['user_ip'])) ? '<span style="color:red; font-weight:bold;">Yes</span>' : '<span style="color:green; font-weight:bold;">No</span>',
					'U_GOOGLE_IP'        => "http://www.google.com/search?q={$user_row['user_ip']}",
					'U_SFS_CHECK'        => $this->u_action . "&amp;action=sfs_check&amp;user_ip={$user_row['user_ip']}",
					'CAN_ADD_ALT'		 => ($userAltData->isMain() ? 1 : 0),
				));

				//Add the mcp language module so we don't have to make new ones.
				$user->add_lang('mcp');
				...
				foreach ($old_emails as $taco)
				{
					$template->assign_block_vars('old_emails', array(
						'OLD_EMAIL'        => ($taco)
					));
				}

				//Include the functions for the custom profile section.
			...
			include($phpbb_root_path . 'includes/functions_user.' . $phpEx);
				...
				{
					list($data['vlastart_day'], $data['vlastart_month'], $data['vlastart_year']) = explode('-', $user_row['user_vla_start']);
				}
				//Check if V/LA end is already defined, if so use that.
				if ($user_row['user_vla_till'])
				{
