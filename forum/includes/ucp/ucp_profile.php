				...
				if ($submit)
				{
					...
					if (!sizeof($error))
					{
						$sql_ary = array(
							...
							'user_old_emails'     => (($auth->acl_get('u_chgemail')) && $data['email'] != $user->data['user_email']) ? (($user->data['user_old_emails'] == '' ? "" : ($user->data['user_old_emails'] . "\n")) . $user->data['user_email']) : ($user->data['user_old_emails']),
						);
