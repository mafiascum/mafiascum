...
class bbcode_firstpass extends bbcode
{
	...
	*/
	function bbcode_size($stx, $in)
	{
		...
		// Do not allow size=0
		// Some browsers glitch with size < 4, so if the size is <= 4 it makes it = 4 instead.
		if ($stx <= 4)
		{
			return '[size=' . 4 . ':' . $this->bbcode_uid . ']' . $in . '[/size:' . $this->bbcode_uid . ']';
		}
	...
	*/
	...
	function bbcode_post($post_number, $in)
	{
		...
		return '[post=#' . $post_number . ':' . $this->bbcode_uid . ']' . $in . '[/post:' . $this->bbcode_uid . ']';
	}
	/**
	 * Parse countdown tag
	 */
	function bbcode_countdown($in)
	{
		...
		if (!$this->check_bbcode('countdown', $in)) {

			return $in;
		}

		else if(preg_match_all('/(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}) ?(-?\d{1,2}\.\d{1,2})?/', $in, $matches, PREG_SET_ORDER)) {
		...
		return '[countdown:' . $this->bbcode_uid . ']' . $in . '[/countdown:' . $this->bbcode_uid . ']';
	}
	...
	function bbcode_dice($dice, $sides, $in, $inExcludingSeed, $operator, $operand, $previousSeed)
	{
		...
		return '[dice:' . $this->bbcode_uid . ']' . $inExcludingSeed . $seedString . '[/dice:' . $this->bbcode_uid . ']';
	}
	...
	*/
	function parse($allow_bbcode, $allow_magic_url, $allow_smilies, $allow_img_bbcode = true, $allow_flash_bbcode = true, $allow_quote_bbcode = true, $allow_url_bbcode = true, $update_this_message = true, $mode = 'post')
	{
		...
		if ($allow_bbcode && strpos($this->message, '[') !== false)
		{
			...
			if($mode == 'sig')
			{
//				$disallow = array('img', 'flash', 'quote', 'url', 'code', 'size', 'list', 'mech', 'area', 'hr', 'dice');
				$disallow = explode("|", $config['disabled_sig_bbcodes']);
				$allow_quote_bbcode = false;
			}
			else
			{
...
function decode_bbcodes_non_preview($message) {

	return preg_replace('/(\[dice.*?\].*?)<!--(\d+)-->(\[\/dice.*?\])/', '$1 $2 $3', $message);
}
